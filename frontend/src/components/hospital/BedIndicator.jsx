...
export function BedIndicator({ label, icon: Icon, available, total, onClick, isSelected }) {
  const isAvailable = (Number(available) || 0) > 0;
  const percentage = total > 0 ? Math.min(100, Math.round(((Number(available) || 0) / total) * 100)) : 0;

  let statusColor = 'bg-emerald-500';
  let textColor = 'text-emerald-600 dark:text-emerald-400';
  if (!isAvailable || percentage === 0) {
  ...
  return (
    <div 
      ...
      title={isAvailable ? `Click to reserve ${label} bed` : `No ${label} beds currently available`}
      className={cn(
        'rounded-xl border p-2 text-center transition-all select-none relative',
        isAvailable 
          ? (onClick ? 'cursor-pointer hover:border-gray-300 dark:hover:border-gray-700' : '') 
          : 'cursor-not-allowed opacity-60 bg-gray-100/60 dark:bg-gray-800/30',
        isSelected && isAvailable
          ? 'border-red-500 bg-red-50/60 dark:bg-red-950/30 shadow-xs ring-1 ring-red-500' 
          : 'border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-card'
      )}
    >
      ...
      <p className="text-[9px] sm:text-[11px] text-muted-foreground font-medium">
        {isAvailable ? `of ${total}` : 'Full / 0 left'}
      </p>
    </div>
  );
}
